...
// Создаёт необходимый формат изобоажения
function MakeDIBPixelFormat(RBitCount, GBitCount, BBitCount: Integer): TDIBPixelFormat;
begin
  Result.RBitMask := ((1 shl RBitCount)-1) shl (GBitCount+BBitCount); // Битовая маска для красного цвета
  Result.GBitMask := ((1 shl GBitCount)-1) shl (BBitCount);
  Result.BBitMask := (1 shl BBitCount)-1;
  Result.RBitCount := RBitCount;   // Сколько бит выделено на интенсивность красного
  ...
  Result.RShift := (GBitCount+BBitCount)-(8-RBitCount); // Кол-во необходимых сдвигов для получения красного
...
begin
  ...
  begin
    ...
    begin
      ...
      OldPalette := 0;
    try
      OldMode := SetStretchBltMode(ACanvas.Handle, FStrBltMode);
      try
        GdiFlush;
...
var
  TempImage: TDIBSharedImage;
  W, H: Integer; Data: Pointer;
  YY, WW, HH: Integer; S, B: ^Byte;
begin
  ...
  If FBitCount = ABitCount then
  begin
   W := FWidthBytes;
   H := FHeight;
   GetMem(Data, W * H);
   Move(FPBits^, Data^, W * H);
   TempImage := TDIBSharedImage.Create;
   try
   ...
   SetImage(TempImage);
   S := Data;
   FillChar(FPBits^, FWidthBytes * FHeight, 0);
   B := FPBits;
   If W < FWidthBytes then WW := W Else WW := FWidthBytes;
   If H < FHeight then HH := H - 1 Else HH := FHeight - 1;
   For YY := 0 to HH do
   begin
    Move(S^, B^, WW);
    Inc(B, FWidthBytes);
...
begin
  ...
  try
    ...
    begin
      { Reduce indexed colors quantity }
    ...
    if (Temp.BitCount<=8) and (BitCount>8) then
    begin
      { Convert RGB or indexed color image to RGB image }
      PaletteToRGB_or_RGBToRGB;
    end else
    if (Temp.BitCount>8) and (BitCount<=8) then
    begin
      { Convert RGB image to indexed color image }
      case BitCount of
        1: begin
             ColorTable[0] := RGBQuad(0, 0, 0);
             ColorTable[1] := RGBQuad(255, 255, 255);
